...
describe("useBankAccounts", () => {
  it("should return an array of bank accounts with activity", async () => {
    global.fetch = jest.fn().mockImplementation((url: string) => {
      if (
        url === "https://api.dev.backpackpay.com/api/v1/mocks/bank-accounts/"
      ) {
        ...
        url ===
        "https://api.dev.backpackpay.com/api/v1/mocks/bank-accounts/1/balance/"
      ) {
        return Promise.resolve({
          status: 200,
          json: () =>
        ...
        url === "https://api.dev.backpackpay.com/api/v1/mocks/transactions/"
      ) {
        return Promise.resolve({
          status: 200,
          json: () =>
    ...
    await waitFor(() =>
      expect(result.current.banckAccountsWithActivity).toHaveLength(1),
    );

    expect(result.current.banckAccountsWithActivity).toEqual(
      testBanckAccountsWithActivity,
    );
  });
  it("should return an array of errors if there is an error fetching bank accounts", async () => {
    ...
    const { result } = renderHook(() => useBankAccounts());
    await waitFor(() =>
      expect(result.current.errors).toContain("Error fetching bank accounts"),
    );
  });
  it("should return an array of errors if there is an error fetching transactions", async () => {
    ...
    const { result } = renderHook(() => useBankAccounts());
    await waitFor(() =>
      expect(result.current.errors).toContain("Error fetching transactions"),
    );
  });
  it("should return an array of errors if there is an error fetching bank account balance", async () => {
    ...
    await waitFor(() =>
      expect(result.current.errors).toContain(
        "Error fetching balance for bank account 1",
      ),
    );
  });
});
