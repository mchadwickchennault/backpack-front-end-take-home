...
export interface UseBankAccountInterface {
    (): {banckAccountsWithActivity: Array<BankAccountWithActivity>, errors: Array<string>}
}
...
export const useBankAccounts : UseBankAccountInterface = () => {
    ...
    useEffect( () => {
        ...
        const fetchBankAccounts = async () => {
            ...
            const bankAccountsResponse = body.data.bank_accounts.map( (bankAccount: BankAccount) => {
                return {
                    ...bankAccount,
        ...
        const fetchTransactions = async () => {
            ...
            const body = await response.json();
            const transactionsResponse = body.data.transactions.map( (transaction: Transaction) => {
                return {
                    ...transaction,
        ...
        if (bankAccounts) {
            for (const bankAccount of bankAccounts) {
                const fetchBalance = async () => {
                    ...
                    if (response.status !== 200) {
                        setErrors([...errors, `Error fetching balance for bank account ${bankAccount.id}`]);
                        return;
                    }
                    const body = await response.json();
