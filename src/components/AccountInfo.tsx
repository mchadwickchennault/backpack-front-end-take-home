...
export function AccountInfo({useBankAccounts}: AccountInfoProps) : React.ReactElement {
            ...
            return (
                <div key={bankAccountWithActivity?.id}>
                    ...
                    <Stack direction="row" spacing={1}>
                        <label>Routing #:</label>
                        <SensitiveInfo value={bankAccountWithActivity?.routing_number || 0}/>
                        <label>Account #:</label>
                        <SensitiveInfo value={bankAccountWithActivity?.account_number || 0 }/>
                    </Stack>
                    <BalanceInfo currentBalanceInCents={bankAccountWithActivity?.balance?.pending_balance_in_cents || 0} availableBalanceInCents={bankAccountWithActivity?.balance?.available_balance_in_cents || 0}/>
                    <TransactionInfo transactions={bankAccountWithActivity.transactions}/>
                </div>
            )
      })}
    </Container>
